...
function PetButton({ src, alt, text, animalType, isSelected, onClick }: PetButtonProps) {
  ...
  return (
    ...
    <div onClick={handleClick} data-animal={animalType}>
      ...
      <div className={`size-[100] bg-[#ffda9c] border-[6px] rounded-[10px] border-[#422206] flex items-center justify-center mb-2 cursor-pointer hover:scale-[104%] ${isSelected && "outline outline-[6px] outline-[#4369fc] border-[#4369fc]"}`}>
        <Image src={src} alt={alt} width={200} height={200} className="size-[60]"/>
      </div>
